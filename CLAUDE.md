...
│
├── grc20-these-mael-rolland-v113.json          # Current authoritative knowledge graph (canonical)
...
The file `grc20-these-mael-rolland-v113.json` is the current authoritative knowledge graph (v113, 2,293 entities, 20,207 relations). Snapshots v96 through v113 are kept in the repo; only the most recent is blocking in CI.

### Entity Types (v113 — 55 types)
...
v113 retyped six author cards from Reference to Person — the residue of the
patch_18 lot — and corrected the **given name** of four of them, each attested
in the thesis bibliography: Audrey → **Adli** Takkal Bataille (l.476), Gregor →
**Andreas** Loibl (l.756), Guillaume → **Gérard** Dréan (l.414), Jérôme →
**Jacques** Favier (l.472). The graph had been attributing invented first names
to four real, living authors. Applied after the author's explicit arbitration of
2026-08-09; no merge, no node, no relation, no attribute touched.
**v113 is the current canonical snapshot.**

### Patch Files

> **Superseded — read this first.** This section describes the **v110**
> inventory. It is kept for its per-file detail, but the live queue is now
> `docs/audits/data/patch-application-queue-current.csv` (**31 artefacts**,
> regenerated by `scripts/build_patch_queue_inventory.py --csv`, four statuses:
> `already_applied`, `indetermine`, `stale_source_graph_but_preconditions_intact`,
> `blocked_by_missing_applicator`). It measures each status **in the current
> graph**, never from what a patch declares — which is how it classes
> `already_applied` the three candidate patches applied by `make_v111`,
> `make_v112` and `make_v113` while they still carry, by CI convention, the
> `CANDIDATE — NOT APPLIED` string.
>
> **The `-current` / `-vNN.snapshot` convention** (author's arbitration Q6,
> 2026-08-09) applies to both queue CSVs: the live file carries `-current`,
> and running a queue script against a *non-current* graph writes
> `…-vNN.snapshot.csv` instead of overwriting the live one. A frozen proof and
> a living inventory must never be able to share a filename. Regenerate the
> live files by hand after each version — the CI step that runs the queue
> tools is **soft**: it checks determinism (same input → same output), not
> freshness, so a stale CSV is not red.
>
> **Governance is a second table.** `patch-queue-governance-cases-current.csv`
> (`scripts/build_patch_queue_governance.py`) answers "what should the repo
> *do* with this artefact?" where the inventory answers "where is it in the
> graph?". Its `recommended_governance_status` derives from the *measured*
> status, never from a patch's `policy`. Seven values, all conservative by
> construction: `archive_historical`, `archive_partial`, `candidate_applied`,
> `blocked_author_arbitration`, `blocked_missing_applicator`, `indetermine`,
> and `ci_candidate` as a separate yes/no column. **Today `ci_candidate` is
> `non` on all 31 rows** — the two technically-applicable artefacts are
> `blocked_author_arbitration`, so the CI has nothing to guard.
>
> **`patch-application-ledger.json`** records what was actually applied
> (patch, applicator, source and applied version, PR, date, declared policy)
> plus a `measured` block re-read from the graph on each run
> (`scripts/build_patch_application_ledger.py`). It exists because a patch's
> own `policy` cannot record its application — C03 forbids removing the
> `CANDIDATE` string. Read the ledger, not the policy.
>
> The full lifecycle policy is `docs/audits/patch-queue-lifecycle-policy-draft.md`
> (a **draft** — not yet enforced anywhere).
...
`patch_candidate_chronology_dates_v1.json`, applied by `make_v112`, a 34th, and
`patch_candidate_bibliographie_retypes_v1.json`, applied by `make_v113`, a 35th.
...
- The knowledge graph JSON (`grc20-these-mael-rolland-v113.json`) is derived from these texts
