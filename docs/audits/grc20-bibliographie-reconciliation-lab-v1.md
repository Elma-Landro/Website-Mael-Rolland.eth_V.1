...
Décompte du CSV (revérifié ligne à ligne, **corrigé par la revue hostile
du 2026-08-07**) : **7 faux absents** (`vraiment_absente=non` — la revue a
attrapé le 7e : NAKAMOTO 2010F, déjà porté par le nœud « Nakamoto 2010g »
mislabellisé, le corps citant en outre ce contenu sous (Nakamoto 2010b) —
trois porteurs pour un même post), 53 vraiment absentes dont **38**
cumulant `entree-complete` + `risque_ajout_inutile=faible` (candidates à
création — Orléan 2002 requalifié de « jamais cité » à « cité » par la
revue : « (Orléan 2002, p. 27) » au ch. II, pagination de l'article),
**6** à risque **élevé** (jamais citées dans le corps — un nœud serait un
orphelin), **6** à risque moyen (entrée complète), **2** entrées
partielles, **1** douteuse. 7+38+6+6+2+1 = 60.

**Prise qui corrige l'audit source** : l'audit § 2.b présentait les 60 entrées comme « sans aucun nœud ». Le diagnostic montre que **6 en ont déjà un** — ce sont des artefacts de clé, pas des absences — et la revue hostile de ce chantier en a attrapé un **7e** (NAKAMOTO 2010F, § ci-dessus, classé `faux-absent` dans le `skipped` du patch missing-nodes), d'où le total de **7** du décompte. Les 6 du diagnostic : BUTERIN 2013D (l'œuvre visée a le nœud `12c586ca…`, l'audit l'avait appariée à 2013a), FRIEDMAN 1999 entrée anonyme (doublon interne du PDF, même URL YouTube), LEIGH STAR & RUHLEDER 2010 (doublon interne du PDF), MANGOLTE 2013B (entrées 2013a/2013b strictement identiques au PDF), NAKAMOTO 2009B (même annonce P2P Foundation que 2009a, URL miroir), POPPER 2016B (le nœud étiqueté « 2016a » porte en réalité le titre de l'article 2016b). Créer ces six nœuds fabriquerait des doublons ; ils sont exclus du patch candidat avec ces raisons.

Autre donnée notable : les « jamais citées » du PDF existent aussi dans ce sens-ci — **6** entrées complètes du PDF (Koning 2013, Nakamoto 2010d/2010e, Song 2018a/2018b, Tual 2016c) n'ont **aucun appel dans le corps** : les créer peuplerait le graphe d'orphelins. Exclues aussi. (Orléan 2002, initialement compté ici, en a été retiré par la revue hostile : l'appel « (Orléan 2002, p. 27) » du ch. II vise bien l'article — la pagination est celle de *L'Homme*, pas du livre d'Aglietta & Orléan.)
...
- **Ne contient pas, et pourquoi** : les 14 `arbitrage-auteur` (aucune preuve bibliographique admissible, ou conflation à corriger d'abord — raison du CSV reprise dans chaque entrée `skipped`) et les 2 `laisser-reference` (fiches à deux auteurs, intypables en une `Person`). Critère d'inclusion strict : seule une fiche dont l'entrée d'auteur du PDF confirme (ou corrige sans ambiguïté ni homonyme) le nom complet est retypée.
...
- **Ne contient pas, et pourquoi** : les 7 faux absents (créer = fabriquer un doublon — dont NAKAMOTO 2010F, attrapé par la revue hostile), les 6 risque élevé (orphelins garantis), les 6 risque moyen (lettrage corps/bibliographie discordant — Nakamoto 2008b —, appel ambigu — Roussel 2017b —, collision de clé — Zelizer 2005 —, etc.), les 2 partielles (URL non pérenne, entrée sans millésime) et la douteuse (THE IRS 2014, titre de l'entrée = celui du guidance FinCEN 2013 déjà noué).
...
- **Égalités de degré** : dans 8 familles fusion-sure (8, 28, 33, 38, 42, 46, 58, 62), le canonique par degré est à égalité avec un autre membre — le critère n'y discrimine pas, la cible proposée est un choix d'ordre, pas une mesure (note `degree_tie_note` du patch duplicates ; couvert par D11). Relevé par la revue hostile.
- **Divergence de types à la fusion** : canonique et doublons peuvent porter des types différents (famille 57 : canonique `Reference` nu, doublon `AcademicWork`) — une fusion naïve appauvrirait le typage ; c'est une troisième décision, distincte de la cible et du libellé (note `type_divergence_note` du patch duplicates). Relevé par la revue hostile.
- **Orphelins potentiels** : les 6 entrées jamais citées du PDF (§ 4), mais aussi, côté inverse, tout retard de câblage des 38 créations candidates — un nœud créé sans ses `cited in` rejoindrait précisément la population « jamais cités » que le diagnostic 4 vient d'assainir.
...
**D15.** Si le patch duplicates est appliqué : régénérer le registre des propriétés (domain/count de `duplicateOf` et `reviewStatus`), statuer sur la coexistence des valeurs `pending-author` / `duplicate-pending-merge`, et vérifier `verif_doublons.py` (§ 6.b). **Arbitré en partie (Arbitrage Queue lot 1, Q4, 2026-08-07)** : réutilisation des clés confirmée, domaine élargi à la régénération du même commit (Q1-A), valeur `pending-author` conservée — les deux valeurs coexisteront, figées par `vocabulary`.
**D16.** Valider (ou élaguer) les **38 créations candidates** du patch missing-nodes — en particulier les trois cas à avertissement : Krugman 2013, Demirgüç-Kunt & Detragiache 1998, Van Wirdum 2018 (§ 7 ; entrees-sans-noeud CSV) — et trancher le rattachement du post Wikileaks aux **trois porteurs** Nakamoto 2010b / 2010g / lettrage 2010f du PDF (7e faux absent, revue hostile).
**D17.** Pour ces créations : choisir la clé d'attribut de provenance, et commander le script `make_vNNN` qui assignera les ids et posera les câblages `cited in` / `appears in section` (§ 6.c). **Arbitré en partie (lot 1, Q3 et Q6, 2026-08-07)** : la clé est **`sourceEntry`** (camelCase, famille `source*` — le patch candidat est déjà renommé) ; le `make_vNNN` dédié sera écrit **après** l'arbitrage des 38 fiches (D16), pas avant.
**D18.** Les 6 entrées complètes du PDF jamais citées dans le corps (Koning 2013, Nakamoto 2010d/e, Song 2018a/b, Tual 2016c) : renoncer à les créer, ou créer sciemment des nœuds de fonds non cités (entrees-sans-noeud, `risque_ajout_inutile=eleve`). Orléan 2002, initialement dans cette liste, a été requalifié « cité » par la revue hostile et rejoint les candidates de D16.
