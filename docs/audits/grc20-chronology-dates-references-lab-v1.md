...
- **Quatre des cinq CSV n'ont aucun générateur.** Seul `chronology-date-inventory-v111.csv` est produit par un script (`audit_chronology_dates.py`, avec `--check`). Les trois relevés qui portent les conclusions les plus lourdes — adossement, doublons, recoupement thèse — ont été **construits à la main : ils ne sont ni rejouables ni contrôlables par `--check`**. L'en-tête « Mécanisme rejouable » ne vaut que pour l'inventaire.
