...
- Graph registers one listener for `layout:apply-requested` and dispatches to existing layout methods:
  ...
  - `applyMatriceLayout`
- Spike cleanup: mediation now reuses the shared `graphe.event-bus.js` pattern (`window.GrapheEventBus`) instead of defining a second inline bus in `graphe.html`.
...
## 9) QA checkpoint — layout-request seam parity gate (2026-04-05)

Scope validated in this spike:
- Mediated path only: `StoryMode.maybeSwitchLayout(layoutTarget)` → `GrapheEventBus.emit('layout:apply-requested', { name, source: 'story-mode' })` → Graph listener dispatch.
- Graph dispatch target methods are unchanged internals:
  - `applyMonetisationLayout`
  - `applyQuiGouverneLayout`
  - `applyMatriceLayout`

Parity-check targets before any further seam work:
- Story-driven layout switch to `monetisation`
- Story-driven layout switch to `qui-gouverne`
- Story-driven layout switch to `matrice`
- Verify `#layout-select` value remains synchronized after each switch

Intentionally still direct / out of scope:
- `applyStoryFocus` and `maybeExpandFromNode`
- Scrolly observer scene-apply path
- Any other Story→Graph or UI→Graph path

Seam-spike containment:
- No additional seam path was added in this spike beyond `layout:apply-requested`.
- `graphe.event-bus.js` is introduced in this branch as the shared bus file used by this spike (not pre-existing runtime infrastructure in earlier baseline commits).
- This spike should stop here pending runtime parity validation.
