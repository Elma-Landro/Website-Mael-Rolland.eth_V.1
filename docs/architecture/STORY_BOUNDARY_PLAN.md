...
## 9. Execution checkpoint — low-risk extraction pass 1 (2026-04-05)

Implemented (behavior-preserving, minimal diff):
- Extracted **registry loading helper** (`loadStoryRegistry`) into `graphe.story-helpers.js`.
- Extracted **alias/focus resolver helper** (`resolveStoryFocusNodes`, with `normalizeStoryText` + alias map) into `graphe.story-helpers.js`.
- Extracted **pure step merge helper** (`mergeStoryStepOptions`) and wired StoryMode step assembly through it.

Explicitly left inline (unchanged):
- `applyStoryFocus` and all `Graph.cy` coupling.
- `maybeExpandFromNode` callback path and Graph node-select seam.
- Scrolly observer wiring and anchored scrolly bridge logic.
- Story panel DOM rendering and mode/layout orchestration.

Rationale:
- Keeps first extraction pass constrained to data-oriented helpers identified as L1/L2/L3.
- Preserves current runtime semantics while preparing cleaner seams for a later extraction branch.

## 10. Execution checkpoint — low-risk extraction pass 2 (2026-04-05)

Implemented (pure render/text helpers only):
- Extracted **reveal button label helper** (`getStoryRevealButtonLabel`).
- Extracted **citation formatting helpers** (`formatStoryCitationText`, `formatStoryCitationPage`).
- Extracted **bridge hint HTML assembly helper** (`buildStoryBridgeHintHtml`).
- Kept all Story panel DOM queries/mutations inline in `updateStoryPanel`; only value/content assembly now delegates to helpers.

Explicitly left inline (unchanged):
- `applyStoryFocus`, `maybeExpandFromNode`, and all `Graph.cy`-dependent Story behavior.
- Scrolly observer and anchored-scrolly wiring.
- Graph/layout methods and mode/layout orchestration.

## 11. Execution checkpoint — low-risk extraction pass 3 (2026-04-05)

Implemented (final pure panel/content helpers):
- Extracted `getStoryPanelEmptyStateText` for default empty panel copy.
- Extracted `formatStoryStepMetaText` for step counter label assembly.
- Extracted `shouldShowStoryBridgeHint` for bridge-hint visibility decision.
- Kept all DOM query/mutation in `updateStoryPanel` inline; helpers provide only pure values/booleans.

Explicitly left inline (unchanged):
- Graph-coupled Story logic (`applyStoryFocus`, `maybeExpandFromNode`, and `Graph.cy` interactions).
- Scrolly observer / anchored-scrolly wiring.
- Graph/layout methods and Story mode orchestration.

Checkpoint note:
- This concludes the planned low-risk helper-only extraction passes before any higher-risk seam work.
